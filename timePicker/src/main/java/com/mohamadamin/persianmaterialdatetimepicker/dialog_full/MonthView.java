...
public abstract class MonthView extends View {
    ...
    public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
        ...
        mDayOfWeekTypeface = res.Utils.getString(R.string.mdtp_day_of_week_label_typeface);
        mMonthTitleTypeface = res.Utils.getString(R.string.mdtp_sans_serif);
    ...
    protected void initView() {
        ...
        mMonthDayLabelPaint.setTypeface(TypefaceHelper.get(getContext(), "iran-sans-light_baz"));
    ...
    protected boolean isFriday(int year, int month, int day) {
        persianCalendar.setPersianDate(year, month, day);
        return persianCalendar.getPersianWeekDayCount() == 6;
    }

    protected boolean isRepeatNoteDay(int year, int month, int day) {
        persianCalendar.setPersianDate(year, month, day);
        ArrayList<String> repeatNoteDays = mController.getRepeatNoteDays();
        String dayName = persianCalendar.getPersianWeekDayName();
        if (repeatNoteDays != null && repeatNoteDays.size() > 0) {
            for (int i = 0; i < repeatNoteDays.size(); i++) {
                if (dayName.equals(repeatNoteDays.get(i)))
                    return true;
            }
        }
        return false;
    ...
    protected boolean isNote(int year, int month, int day) {
        ArrayList<String> noteDates = mController.getNoteDate();
        if (noteDates != null && noteDates.size() == 0) return false;
        for (int i = 0; i < noteDates.size(); i++) {

            String[] date = noteDates.get(i).split("\\/");
            int mYear = Integer.parseInt(date[0].toString());
            int mMonths = Integer.parseInt(date[1].toString()) - 1;
            int mDays = Integer.parseInt(date[2].toString());


            if (year < mYear) break;
            if (year > mYear) continue;
            if (month < mMonths) break;
            if (month > mMonths) continue;
            if (day < mDays) break;
            if (day > mDays) continue;
            return true;
        }
        return false;
    }

    protected boolean isHoliday(int year, int month, int day) {
        ArrayList<String> HighlightedDate = mController.getHighlightedDate();
        if (HighlightedDate != null && HighlightedDate.size() == 0) return false;
        for (int i = 0; i < HighlightedDate.size(); i++) {

            String[] date = HighlightedDate.get(i).split("\\/");
    ...
    protected class MonthViewTouchHelper extends ExploreByTouchHelper {
        ...
        protected CharSequence getItemDescription(int day) {
            ...
            if (day == mSelectedDay) {
                return getContext().Utils.getString(R.string.mdtp_item_is_selected, date);
            }
